...
def transform(vcam, transform_matrix):
    ...
    linear_velocity_robot = np.dot(rotation_matrix, vcam[:3]) + np.cross(translation_vector, np.dot(rotation_matrix, vcam[3:]))

    # Transform the angular velocity
    angular_velocity_robot = np.dot(rotation_matrix, vcam[3:])
...
class MyPythonNode(Node):
    ...
    def trackerCallback(self, data):
        ...
        error_vs = [self.tracked_point[0] - self.desired_point[0], self.tracked_point[1] - self.desired_point[1]]
        # Calculate the interaction matrix
        currentL = interactionMatrix(self.tracked_point)
        ...
        homogeneous_transform_from_cam_to_robot = np.array([
            [1, 0, 0, 0.2],  # 20 cm offset in the x-axis
            [0, 1, 0, 0],    # No offset in the y-axis
            [0, 0, 1, 0],    # No offset in the z-axis
